...
impl App {
    ...
    fn run(&mut self, terminal: &mut Terminal<CrosstermBackend<io::Stdout>>) -> io::Result<()> {
        while !self.exit {
            while let Ok(order) = self.order_rx.try_recv() {
                // If receive order from simulator, submit to order book
    ...
    fn submit_trade(&mut self, order: Order) -> Vec<Event>{
        ...
        if let (Some((best_bid, bid_quantity)), Some((best_ask, ask_quantity))) = (best_bid_info, best_ask_info) {
    ...
    fn sort_book_level(book: &BTreeMap<u64, Vec<Order>>, ascending: bool) -> Vec<(u64, u64)> {
        // Sort by price, returns (price, quantity)
        let mut levels: Vec<(u64, u64)> = book.iter()
    ...
    fn get_max_quantity(book: &BTreeMap<u64, Vec<Order>>) -> u64 {
        book.values()
